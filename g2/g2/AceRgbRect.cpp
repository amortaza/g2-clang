...
void AceRgbRect::draw(AceProgram* prog, int left, int top, int w, int h, glm::mat4* Projection, float* alphas, float *reds, float *greens, float *blues) {
    prog->activate();
...
void AceRgbRect::setVertexData(int left, int top, int w, int h, float* alphasLeftTopRightBottom, float *reds, float *greens, float *blues) {
    int right = left + w;
    int bottom = top + h;        
